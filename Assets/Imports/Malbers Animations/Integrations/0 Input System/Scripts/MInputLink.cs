...
namespace MalbersAnimations
{
    ...
    public enum MInputInteraction { Press = 0, Down = 1, Up = 2, LongPress = 3, DoubleTap = 4, Toggle = 5, SingleValue = 6, Charge = 7 }
    ...
    public class MInputAction : IInputAction
    {
        ...
        public FloatEvent OnInputUpValue = new FloatEvent();
        ...
        public void TranslateInput(InputAction.CallbackContext context)
        {
            ...
            switch (interaction)
            {
				...
				case MInputInteraction.Charge:
                    ...
                    {
                        InputCompleted = true;
                        OnInputUpValue.Invoke(InputElapsed);
                        OnInputUp.Invoke();
                        OnInputChanged.Invoke(InputValue = true);
                        MCoroutine.StartCoroutine(IEnum_UpRelease());
                        InputElapsed = 0f;
		...
		{
            if (C_Charge != null)
                MCoroutine.StopCoroutine(C_Charge);

            C_Charge = IEnum_Charge();
            MCoroutine.StartCoroutine(C_Charge);
		}
        ...
        IEnumerator IEnum_Charge()
		{
            ...
            while (!InputCompleted)
            {
                InputElapsed = (Time.time - InputStartTime);
                OnInputFloatValue?.Invoke(InputElapsed);
                yield return null;
            }
            InputCompleted = false;
        }
            ...
            EditorGUILayout.BeginVertical(EditorStyles.helpBox);
            {
                ...
                if (Element.isExpanded)
                {
                    ...
                    switch (interaction)
                    {
                        ...
                        case MInputInteraction.Charge:
                            EditorGUILayout.PropertyField(OnInputDown, new GUIContent("On Input Down"));
                            EditorGUILayout.PropertyField(Element.FindPropertyRelative("OnInputFloatValue"), new GUIContent("On Charge"));
                            EditorGUILayout.PropertyField(Element.FindPropertyRelative("OnInputUpValue"), new GUIContent("On Charge Released"));
                            EditorGUILayout.PropertyField(OnInputUp, new GUIContent("On Pressed Interrupted"));
                            EditorGUILayout.PropertyField(OnInputChanged);
                            break;
                        default:
